...
#                    continue
#                if input_name == 'evdist_1' and weight in ['L2Pyr_ampa', 'L2Basket_ampa', 'L2Basket_nmda', 'L5Basket_ampa', 'L5Basket_nmda', 'L5Pyr_ampa']:
#                    continue
#                if input_name == 'evprox_2' and weight in ['L2Pyr_ampa', 'L2Basket_ampa', 'L2Basket_nmda', 'L5Basket_ampa']:
#                    continue
...
for index, chunk in enumerate(chunks):
    ...
    opt_results = optimize(params_input, 500)
...
params_input['sim_prefix'] = \
    "%s_all_%s_%s" % (
                      op.basename(params_fname).split('.json')[0],
                      include_weights, environ['ALGORITHM'])
params_input['opt_start'] = 0.0
params_input['opt_end'] = params_input['tstop'] = chunks[-1]['end']
params_input['opt_params'] = set_parameters(include_weights, all_inputs_to_opt)
print('optimizing from [%3.3f-%3.3f]' % (params_input['opt_start'], params_input['opt_end']))
num_params = len(params_input['opt_params'])
opt = nlopt.opt(algorithm, num_params)
opt_results = optimize(params_input, 500)
