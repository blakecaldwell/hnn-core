...
try:
    ...
    (extdata, base_params_input, param_names) = comm.bcast(rank, root=0)
...
#    params_fname = op.join(mne_neuron_root, 'param', 'default.json')
    params_fname = "/users/bcaldwe2/delfi/051220_peribeta_basedonpostbeta16_6_opt_0.05_timestep.json"
...
while True:
    ...
    if get_rank() == 0:
        ...
        #avg_rmse = avg_dpl.rmse(exp_dpl, tstart, params['tstop'])
        ...
        if loop:
            ...
            numspikes = len(net.spiketimes)

            # downsample simulation timeseries to match exp data
            dpl1 = signal.resample(dpl1, exp_length)
            data = np.array([dpl1,
                            [numspikes, params['tstop'], task_index]])
            comm.send(data, dest=0)

            # write params to file with RMSE
            #params.write("/users/bcaldwe2/scratch/delfi/sim_params/%s_sim_%d.json" % (params['sim_prefix'], int(task_index)))
